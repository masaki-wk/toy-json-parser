...
fn parse_array_single_item() -> Result<()> {
    let input = "[0]";
    let start = CodePos::new(1, 1);
    let literal_start = CodePos::new(start.line, start.column + 1);
...
fn parse_array_multiple_item() -> Result<()> {
    let input = "[0, 1]";
    let start = CodePos::new(1, 1);
    let literal1_start = CodePos::new(start.line, start.column + 1);
    let literal1_end = CodePos::new(literal1_start.line, literal1_start.column + 1);
    let literal2_start = CodePos::new(literal1_end.line, literal1_end.column + 2);
...
fn parse_object_single_pair() -> Result<()> {
    let input = "{\"a\": 0}";
    let start = CodePos::new(1, 1);
    let name_start = CodePos::new(start.line, start.column + 1);
    let name_end = CodePos::new(name_start.line, name_start.column + 3);
    let value_start = CodePos::new(name_end.line, name_end.column + 2);
    let value_end = CodePos::new(value_start.line, value_start.column + 1);
    let end = CodePos::new(start.line, start.column + input.chars().count());
    let mut buf = Vec::new();
    buf.push((
...
fn parse_object_multiple_pair() -> Result<()> {
    let input = "{\"a\": 0, \"b\": 1}";
    let start = CodePos::new(1, 1);
    let name1_start = CodePos::new(start.line, start.column + 1);
    let name1_end = CodePos::new(name1_start.line, name1_start.column + 3);
    let value1_start = CodePos::new(name1_end.line, name1_end.column + 2);
    let value1_end = CodePos::new(value1_start.line, value1_start.column + 1);
    let name2_start = CodePos::new(value1_end.line, value1_end.column + 2);
    let name2_end = CodePos::new(name2_start.line, name2_start.column + 3);
    let value2_start = CodePos::new(name2_end.line, name2_end.column + 2);
    let value2_end = CodePos::new(value2_start.line, value2_start.column + 1);
    let end = CodePos::new(start.line, start.column + input.chars().count());
    let mut buf = Vec::new();
    buf.push((
...
fn parse_illegal_extra_token_at_the_end() -> Result<()> {
    ...
    let pos = CodePos::new(1, 1 + body.chars().count());
...
fn parse_illegal_object_name_is_not_string() -> Result<()> {
    ...
    let start = CodePos::new(1, 1);
    let end = CodePos::new(start.line, start.column + pre.chars().count());
